  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4">
      <div className="grid grid-cols-2 gap-2">
        <BodySilhouette label="Front" color={color} usage={usage} view="front" />
        <BodySilhouette label="Back" color={color} usage={usage} view="back" />
      </div>
      <ul className="mt-3 grid grid-cols-2 gap-x-3 gap-y-1 text-sm">
        {SITES.map((s) => (
          <li key={s} className="flex justify-between">
            <span className={s === mostUsed ? "font-semibold text-danger" : ""}>{s}</span>
      ...
      {mostUsed && (
        <p className="mt-3 rounded-md bg-danger/10 p-2 text-xs text-danger">
          Avoid injecting in <strong>{mostUsed}</strong> for a few days to prevent
          lipohypertrophy.
        </p>
      )}
    </div>
